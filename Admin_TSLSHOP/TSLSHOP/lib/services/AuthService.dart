...
class AuthService {
      ...
      if (await userService.isUserExist(value.user!.email, LoginTypeApp)) {
        await userService.userByEmail(email).then((user) async {
          if (user.role == ADMIN || user.role == REST_MANAGER) {
            ...
            throw '${appStore.translate('you_are_not_registered_with_us')}${user.role}';
          }
        }).catchError((e) {
          log(e.toString());
          throw e;
        ...
        throw appStore.translate('You_are_already_registered');
      }
    }).catchError((error) async {
      if (!await isNetworkAvailable()) {
        throw appStore.translate('Please_check_network_connection');
      }
      log(error.toString());
      throw appStore.translate('Enter_valid_email_and_password');
    });
  }
  ...
  Future<void> signOutFromEmailPassword(BuildContext context) async {
    ...
    await removeKey(RESTAURANT_ID);
    await removeKey(RESTAURANT_NAME);
    await removeKey(RESTAURANT_CITY);
