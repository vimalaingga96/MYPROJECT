...
class OrderService extends BaseService {
  ...
  Stream<List<OrderModel>> getOrders({String? id, List<String>? orderStatus, bool isDash = false}) {
    ...
    if (getBoolAsync(IS_ADMIN) && isDash) {
      ...
      query = ref!.where(OrderKey.restaurantId, isEqualTo: id).where(OrderKey.orderStatus, whereIn: orderStatus).orderBy(TimeDataKey.createdAt, descending: true);
    }
  ...
  Stream<List<OrderModel>> restaurantTotalAmount({DateTime? startDate}) {
    return ref!
        .where(OrderKey.restaurantId, isEqualTo: getStringAsync(RESTAURANT_ID))
