...
class CompletedOrderScreenState extends State<CompletedOrderScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarWidget(appStore.translate('completed_order'), showBack: false, elevation: 0),
      body: StreamBuilder<List<OrderModel>>(
        stream: orderService.getOrders(id: getStringAsync(RESTAURANT_ID), orderStatus: [COMPLETED]),
        builder: (_, snap) {
          if (snap.hasData) {
