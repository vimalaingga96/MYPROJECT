            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  8.width,
                  Text(widget.data!.type == 1 ? appStore.translate('approved') : appStore.translate('pending'),
              ...
              Row(
                children: [
                  ...
                  8.width,
                  Text(
                    !widget.data!.isDeleted.validate() ? appStore.translate('active') : appStore.translate('inActive'),
                    style: primaryTextStyle(color: !widget.data!.isDeleted.validate() ? Colors.green : Colors.red),
                  ).onTap(() async {
                    bool? res = await showConfirmDialog(
                      context,
                      widget.data!.isDeleted.validate() ? appStore.translate('active_user') : appStore.translate('inactive_user'),
              ...
              Visibility(
                ...
                child: Row(
                  children: [
                    Text(appStore.translate('accept'), style: primaryTextStyle(color: Colors.green)).onTap(() {
                      if (getBoolAsync(IS_TESTER)) {
                        return toast(appStore.translate('mTesterNotAllowedMsg'));
                      } else {
                        isAccept = 1;
                    ...
                    8.width,
                    Text(appStore.translate('reject'), style: primaryTextStyle(color: Colors.redAccent)).onTap(() {
                      if (getBoolAsync(IS_TESTER)) {
                        return toast(appStore.translate('mTesterNotAllowedMsg'));
                      } else {
                        isAccept = 0;
